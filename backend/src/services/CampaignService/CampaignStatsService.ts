...
const CampaignStatsService = async (campaignId: string | number): Promise<CampaignStats> => {
  try {
    ...
    if (isEmailCampaign) {
      const emailTotal = await CampaignShipping.count({
        where: { campaignId }
      });

      const emailSent = await CampaignShipping.count({
        where: { campaignId, emailStatus: "sent" }
      });

      const emailFailed = await CampaignShipping.count({
        where: { campaignId, emailStatus: "failed" }
      });

      const emailOpened = await CampaignShipping.count({
        where: { campaignId, emailStatus: "opened" }
      });

      const emailClicked = await CampaignShipping.count({
        where: { campaignId, emailStatus: "clicked" }
      });

      const emailBounced = await CampaignShipping.count({
        where: { campaignId, emailStatus: "bounced" }
      });

      const emailUnsubscribed = await CampaignShipping.count({
        where: { campaignId, emailStatus: "unsubscribed" }
      });

      return {
        totalMessages: emailTotal,
        deliveredMessages: emailSent,
        pendingMessages: Math.max(0, totalContacts - emailTotal),
        failedMessages: emailFailed,
        uniqueNumbers: totalContacts,
        deliveryRate: totalContacts > 0 ? Math.round((emailSent / totalContacts) * 10000) / 100 : 0,
        ...
        emailBounced,
        emailUnsubscribed,
        emailOpenRate: emailSent > 0 ? Math.round((emailOpened / emailSent) * 10000) / 100 : 0,
        emailClickRate: emailSent > 0 ? Math.round((emailClicked / emailSent) * 10000) / 100 : 0
      };
    }
