...
import VerifyEmailCodeService from "../services/UserServices/VerifyEmailCodeService";
import ResendVerificationCodeService from "../services/UserServices/ResendVerificationCodeService";
  ...
  if (!companyUser) {
    ...
    try {
      ...
      if (
        ...
        const body =
          `Olá ${name}, seu cadastro da empresa ${companyName} foi iniciado com sucesso!\n\n` +
          `Para concluir o acesso ao sistema, confirme o código enviado para o e-mail ${email}.\n\n` +
          `Após a confirmação do código, você poderá entrar normalmente na plataforma.\n\n` +
      ...
      logger.warn(
        "[USER] Não consegui enviar a mensagem de confirmação de cadastro via WhatsApp"
      );
    }

    const companyJson = user.toJSON ? user.toJSON() : user;
...
export const verifyEmail = async (
  ...
  const user = await VerifyEmailCodeService({
    email,
    code
  });
  ...
  if (user.companyId) {
    const io = getIO();
    io.of(String(user.companyId)).emit(`company-${user.companyId}-user`, {
      action: "update",
      user
    });
  }
