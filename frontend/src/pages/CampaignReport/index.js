...
const CampaignReport = () => {
  ...
  // Função getRecipient - adicionada abaixo de formatPhoneNumber
  const getRecipient = (item) => {
    if (campaign?.campaignType === "email") {
      return item.email || item.contact?.email || "";
    }
    return item.number || "";
  };
    ...
    try {
      ...
      try {
        ...
        if (shippingResponse && shippingResponse.shipping) {
          ...
          // Processar dados para a tabela - ALTERADO: adicionado email
  ...
  const exportToExcel = async () => {
    try {
      ...
      // Adicionar dados das mensagens - ALTERADO: usando campaign?.campaignType
              ...
              <TableBody>
                {messageRows.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell>
                      <Tooltip title={getRecipient(row)}>
                        <span>
                          {campaign?.campaignType === "email"
                            ? getRecipient(row)
