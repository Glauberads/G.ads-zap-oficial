...
export function CompaniesManagerGrid(props) {
  ...
  const renderCreatedAt = (row) => {
    if (!row.createdAt || !moment(row.createdAt).isValid()) {
      return "-";
    }
  ...
  const renderLastLogin = (row) => {
    if (!row.lastLogin || !moment(row.lastLogin).isValid()) {
      return "-";
    }
    return moment(row.lastLogin).format("DD/MM/YYYY HH:mm");
  };
        ...
        <TableBody>
          {records.map((row, key) => (
            <TableRow style={rowStyle(row)} key={key}>
              ...
              <TableCell style={cellStyle(row)} align="center">{renderCreatedAt(row)}</TableCell>
              <TableCell style={cellStyle(row)} align="center">{dateToClient(row.dueDate)}<br /><span>{row.recurrence}</span></TableCell>
              <TableCell style={cellStyle(row)} align="center">{renderLastLogin(row)}</TableCell>
